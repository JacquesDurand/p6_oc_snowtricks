    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasherInterface, FileUploader $fileUploader): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->emailVerifier->sendEmailConfirmation(
                ...
                $user,
                (new TemplatedEmail())
                    ->from(new Address($this->getParameter('mail_to'), 'SnowTricks Admin'))
                    ->to($user->getEmail())
...
//             do anything else you need here, like send an email
            ...
            $this->addFlash(
                'success',
                'Your registration was successful, an email has been sent to '.$user->getEmail().' to verify your address'
            );
            $this->addFlash(
                'warning',
                'Note that you have to be logged in first to verify your email !'
            );
